...
def weatherforecast():
    ...
    next_5_hours = currentime + timedelta(hours=5)
    forecastdf = forecastdf[(forecastdf["datetime"] >= currentime) & (forecastdf["datetime"] <= next_5_hours)]  # filter for next 5 hours only
